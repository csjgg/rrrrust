...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let m = seq1.len();
    let n = seq2.len();
    let mut nowgrid = Grid::new(m + 1, n + 1);
    for i in 0..m + 1 {
        let _ = nowgrid.set(i, 0, 0);
    }
    for j in 0..n + 1 {
        let _ = nowgrid.set(0, j, 0);
    }
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                let _ = nowgrid.set(i + 1, j + 1, nowgrid.get(i, j).unwrap()+1);
            } else {
                let _ = nowgrid.set(
                    i + 1,
                    j + 1,
                    max(
                        nowgrid.get(i + 1, j).unwrap(),
                        nowgrid.get(i, j + 1).unwrap(),
                    ),
                );
            }
        }
...
fn main() {
    ...
    let nowgrid = lcs(&seq1, &seq2);
    print_diff(&nowgrid, &seq1, &seq2, seq1.len(), seq2.len());
}
